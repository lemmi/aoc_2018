...
fn bounding_box(points: &Vec<Point>, fatten: isize) -> (Point, Point) {
    let minx = points.iter().map(|p| p.x).min().expect("Error finding bounding box");
    let miny = points.iter().map(|p| p.y).min().expect("Error finding bounding box");
    let maxx = points.iter().map(|p| p.x).max().expect("Error finding bounding box");
    let maxy = points.iter().map(|p| p.y).max().expect("Error finding bounding box");

    (Point::new(minx - fatten, miny - fatten), Point::new(maxx + fatten, maxy + fatten))
}
...
fn closest_to(points: &Vec<Point>, p: Point) -> Option<Point> {
    let c = points.iter().min_by_key(|ps| ps.dist(&p)).expect("Error finding closest Point");
    for ps in points {
        if ps == c {
            continue;
        }
...
fn within_box(bb: (Point, Point)) -> impl Iterator<Item = (isize,isize)> {
    ...
    (ul.x..lr.x+1).cycle().zip(
        (ul.y..lr.y+1)
        .flat_map(move |y| std::iter::repeat(y).take(w))
        )
}
...
pub fn star1(lines: impl Iterator<Item = io::Result<String>>) -> StarResult {
    ...
    let (p, a) = cover.iter().max_by_key(|(_, a)| *a).expect("Expected a Point with maximum coverage");
...
pub fn star2(lines: impl Iterator<Item = std::io::Result<String>>) -> super::StarResult {
    ...
    println!("Size of the area of locations less then 10000 away is {}", area);
