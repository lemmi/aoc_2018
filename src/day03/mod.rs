...
impl std::str::FromStr for Rec {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        Ok(Rec::new(tokens[1], tokens[2], tokens[3], tokens[4], Some(tokens[0])))
    }
}

impl Rec {
    fn new(x: usize, y: usize, w: usize, h: usize, id: Option<usize>) -> Rec {
            Rec{ x: x, y: y, w: w, h: h, id: id}
    }
    fn area(&self) -> usize {
        self.w * self.h
    }
    ...
    fn intersect(&self, other: &Rec) -> Option<Rec> {
        ...
        if w < 1 || h < 1 {
            ...
            Some(Rec::new(left as usize, upper as usize, w as usize, h as usize, None))
        }
    }
...
pub fn star1(lines: impl Iterator<Item = io::Result<String>>) -> super::StarResult {
    let boxes = lines.map(|l| l.unwrap()).map(|s| s.parse::<Rec>()).collect::<Result<Vec<Rec>,_>>()?;
    ...
    println!("{} squares are covered, with over {} claims ({})", cover, claims, area);
